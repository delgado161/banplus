...
// Create page object
$banner_seccion_edit = new cbanner_seccion_edit();
$Page =& $banner_seccion_edit;

// Page init
$banner_seccion_edit->Page_Init();

// Page main
$banner_seccion_edit->Page_Main();
?>
...
// Create page object
var banner_seccion_edit = new ew_Page("banner_seccion_edit");

// page properties
banner_seccion_edit.PageID = "edit"; // page ID
banner_seccion_edit.FormID = "fbanner_seccionedit"; // form ID
var EW_PAGE_ID = banner_seccion_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
banner_seccion_edit.ValidateForm = function(fobj) {
...
banner_seccion_edit.Form_CustomValidate =  
...
banner_seccion_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
banner_seccion_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $banner_seccion->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $banner_seccion->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $banner_seccion_edit->ShowPageHeader(); ?>
<?php
$banner_seccion_edit->ShowMessage();
?>
<form name="fbanner_seccionedit" id="fbanner_seccionedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return banner_seccion_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="banner_seccion">
...
<input type="hidden" name="x_id_banner_seccion" id="x_id_banner_seccion" value="<?php echo ew_HtmlEncode($banner_seccion->id_banner_seccion->CurrentValue) ?>">
<p>
<input type="submit" name="btnAction" id="btnAction" value="<?php echo ew_BtnCaption($Language->Phrase("EditBtn")) ?>">
</form>
<?php
$banner_seccion_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$banner_seccion_edit->Page_Terminate();
?>
<?php
...
class cbanner_seccion_edit {
	...
	// Page object name
	var $PageObjName = 'banner_seccion_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$banner_seccion->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$banner_seccion->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$banner_seccion->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$banner_seccion->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($banner_seccion->id_banner_seccion->CurrentValue == "")
			$this->Page_Terminate("banner_seccionlist.php"); // Invalid key, return to list
		switch ($banner_seccion->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$banner_seccion->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$banner_seccion->RowType = EW_ROWTYPE_EDIT; // Render as Edit
	...
	function LoadFormValues() {
		...
		if (!$banner_seccion->id_banner_seccion->FldIsDetailKey)
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// link
			$banner_seccion->link->SetDbValueDef($rsnew, $banner_seccion->link->CurrentValue, NULL, $banner_seccion->link->ReadOnly);

			// externo
			$banner_seccion->externo->SetDbValueDef($rsnew, $banner_seccion->externo->CurrentValue, NULL, $banner_seccion->externo->ReadOnly);
