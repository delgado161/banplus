...
class cestado {
	...
	function cestado() {
		...
		// id_estado
		$this->id_estado = new cField('estado', 'estado', 'x_id_estado', 'id_estado', '`id_estado`', 3, -1, FALSE, '`id_estado`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_estado->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_estado'] =& $this->id_estado;

		// nombre
		$this->nombre = new cField('estado', 'estado', 'x_nombre', 'nombre', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nombre'] =& $this->nombre;

		// estatus
		$this->estatus = new cField('estado', 'estado', 'x_estatus', 'estatus', '`estatus`', 3, -1, FALSE, '`estatus`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `estado` ($names) VALUES ($values)";
	}
	...
	function DeleteSQL(&$rs) {
		...
		$SQL .= ew_QuotedName('id_estado') . '=' . ew_QuotedValue($rs['id_estado'], $this->id_estado->FldDataType) . ' AND ';
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->id_estado->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_estado@", ew_AdjustSql($this->id_estado->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("estadoview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "estadoadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("estadoedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("estadoadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("estadodelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_estado->CurrentValue)) {
			$sUrl .= "id_estado=" . urlencode($this->id_estado->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=estado" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["id_estado"]; // id_estado
   ...
   // Common render codes
		...
		$this->id_estado->CellCssStyle = "white-space: nowrap;";
