...
//
// Table class for usuariopermisos
//
class cusuariopermisos {
	var $TableVar = 'usuariopermisos';
	...
	//
	function cusuariopermisos() {
		global $Language;
		$this->AllowAddDeleteRow = ew_AllowAddDeleteRow(); // Allow add/delete row

		// userlevelid
		$this->userlevelid = new cField('usuariopermisos', 'usuariopermisos', 'x_userlevelid', 'userlevelid', '`userlevelid`', 3, -1, FALSE, '`userlevelid`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->userlevelid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['userlevelid'] =& $this->userlevelid;

		// tablename
		$this->ztablename = new cField('usuariopermisos', 'usuariopermisos', 'x_ztablename', 'tablename', '`tablename`', 200, -1, FALSE, '`tablename`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['tablename'] =& $this->ztablename;

		// permission
		$this->permission = new cField('usuariopermisos', 'usuariopermisos', 'x_permission', 'permission', '`permission`', 3, -1, FALSE, '`permission`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function HighlightName() {
		return "usuariopermisos_Highlight";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `usuariopermisos` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `usuariopermisos` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `usuariopermisos` WHERE ";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "usuariopermisoslist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("usuariopermisosview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "usuariopermisosadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("usuariopermisosedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("usuariopermisosadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("usuariopermisosdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=usuariopermisos" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKey[] = @$_GET["ztablename"]; // tablename
			$arKeys[] = $arKey;
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->ztablename->CurrentValue = $key[1];
   ...
   // Common render codes
		...
		// permission
		if (strval($this->permission->CurrentValue) <> "") {
			$arwrk = explode(",", $this->permission->CurrentValue);
			$sFilterWrk = "";
		...
		if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				$this->permission->ViewValue = "";
				$ari = 0;
				while (!$rswrk->EOF) {
					$this->permission->ViewValue .= $rswrk->fields('userlevelname');
					$rswrk->MoveNext();
					if (!$rswrk->EOF) $this->permission->ViewValue .= ew_ViewOptionSeparator($ari); // Separate Options
					$ari++;
				}
				$rswrk->Close();
			} else {
			...
			$this->permission->ViewValue = NULL;
		}
		...
		// tablename
		$this->ztablename->LinkCustomAttributes = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('ztablename', $this->ztablename->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('permission', $this->permission->ExportValue($this->Export, $this->ExportOriginalValue));
				} else {
					$XmlDoc->AddField('userlevelid', $this->userlevelid->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('ztablename', $this->ztablename->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('permission', $this->permission->ExportValue($this->Export, $this->ExportOriginalValue));
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->ztablename);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->ztablename);
