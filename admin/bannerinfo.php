...
class cbanner {
	...
	function cbanner() {
		...
		// id_banner
		$this->id_banner = new cField('banner', 'banner', 'x_id_banner', 'id_banner', '`id_banner`', 3, -1, FALSE, '`id_banner`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_banner->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_banner'] =& $this->id_banner;

		// nombre
		$this->nombre = new cField('banner', 'banner', 'x_nombre', 'nombre', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nombre'] =& $this->nombre;

		// imagen
		$this->imagen = new cField('banner', 'banner', 'x_imagen', 'imagen', '`imagen`', 200, -1, TRUE, '`imagen`', FALSE, FALSE, 'IMAGE');
		$this->imagen->UploadPath = '../uploads/banners';
		$this->fields['imagen'] =& $this->imagen;

		// estatus
		$this->estatus = new cField('banner', 'banner', 'x_estatus', 'estatus', '`estatus`', 3, -1, FALSE, '`estatus`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "banner_seccion") {
			$sDetailUrl = $GLOBALS["banner_seccion"]->ListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&id_banner=" . $this->id_banner->CurrentValue;
		}
		return $sDetailUrl;
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `banner` ($names) VALUES ($values)";
	}
	...
	function DeleteSQL(&$rs) {
		...
		$SQL .= ew_QuotedName('id_banner') . '=' . ew_QuotedValue($rs['id_banner'], $this->id_banner->FldDataType) . ' AND ';
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->id_banner->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_banner@", ew_AdjustSql($this->id_banner->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("bannerview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "banneradd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("banneredit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("banneredit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function CopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("banneradd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("banneradd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("bannerdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_banner->CurrentValue)) {
			$sUrl .= "id_banner=" . urlencode($this->id_banner->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=banner" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["id_banner"]; // id_banner
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_banner->CurrentValue = $key;
   ...
   // Common render codes
		...
		if (!ew_Empty($this->imagen->Upload->DbValue)) {
			...
			$this->imagen->ImageWidth = 350;
			$this->imagen->ImageHeight ="";
