...
//
// Table class for informacion_financiera
//
class cinformacion_financiera {
	var $TableVar = 'informacion_financiera';
	var $TableName = 'informacion_financiera';
	var $TableType = 'TABLE';
	var $id_informacion_financiera;
	var $ano;
	var $mes;
	var $archivo;
	var $estatus;
	...
	//
	function cinformacion_financiera() {
		global $Language;
		$this->AllowAddDeleteRow = ew_AllowAddDeleteRow(); // Allow add/delete row

		// id_informacion_financiera
		$this->id_informacion_financiera = new cField('informacion_financiera', 'informacion_financiera', 'x_id_informacion_financiera', 'id_informacion_financiera', '`id_informacion_financiera`', 3, -1, FALSE, '`id_informacion_financiera`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_informacion_financiera->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_informacion_financiera'] =& $this->id_informacion_financiera;

		// ano
		$this->ano = new cField('informacion_financiera', 'informacion_financiera', 'x_ano', 'ano', '`ano`', 3, -1, FALSE, '`ano`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->ano->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ano'] =& $this->ano;

		// mes
		$this->mes = new cField('informacion_financiera', 'informacion_financiera', 'x_mes', 'mes', '`mes`', 200, -1, FALSE, '`mes`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['mes'] =& $this->mes;

		// archivo
		$this->archivo = new cField('informacion_financiera', 'informacion_financiera', 'x_archivo', 'archivo', '`archivo`', 200, -1, TRUE, '`archivo`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->archivo->UploadPath = '../uploads/estadofinanciero';
		$this->fields['archivo'] =& $this->archivo;

		// estatus
		$this->estatus = new cField('informacion_financiera', 'informacion_financiera', 'x_estatus', 'estatus', '`estatus`', 3, -1, FALSE, '`estatus`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function HighlightName() {
		return "informacion_financiera_Highlight";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `informacion_financiera` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `informacion_financiera` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `informacion_financiera` WHERE ";
		$SQL .= ew_QuotedName('id_informacion_financiera') . '=' . ew_QuotedValue($rs['id_informacion_financiera'], $this->id_informacion_financiera->FldDataType) . ' AND ';
	...
	function SqlKeyFilter() {
		return "`id_informacion_financiera` = @id_informacion_financiera@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->id_informacion_financiera->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_informacion_financiera@", ew_AdjustSql($this->id_informacion_financiera->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "informacion_financieralist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("informacion_financieraview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "informacion_financieraadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("informacion_financieraedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("informacion_financieraadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("informacion_financieradelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_informacion_financiera->CurrentValue)) {
			$sUrl .= "id_informacion_financiera=" . urlencode($this->id_informacion_financiera->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=informacion_financiera" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["id_informacion_financiera"]; // id_informacion_financiera
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_informacion_financiera->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		$this->id_informacion_financiera->setDbValue($rs->fields('id_informacion_financiera'));
   ...
   // Common render codes
		...
		$this->id_informacion_financiera->ViewValue = $this->id_informacion_financiera->CurrentValue;
		$this->id_informacion_financiera->ViewCustomAttributes = "";
		...
		// id_informacion_financiera
		$this->id_informacion_financiera->LinkCustomAttributes = "";
		$this->id_informacion_financiera->HrefValue = "";
		$this->id_informacion_financiera->TooltipValue = "";
		...
		if (!ew_Empty($this->archivo->Upload->DbValue)) {
			...
			if ($this->Export <> "") $informacion_financiera->archivo->HrefValue = ew_ConvertFullUrl($this->archivo->HrefValue);
		} else {
