...
// Create page object
$z_estados_add = new cz_estados_add();
$Page =& $z_estados_add;

// Page init
$z_estados_add->Page_Init();

// Page main
$z_estados_add->Page_Main();
?>
...
// Create page object
var z_estados_add = new ew_Page("z_estados_add");

// page properties
z_estados_add.PageID = "add"; // page ID
z_estados_add.FormID = "fz_estadosadd"; // form ID
var EW_PAGE_ID = z_estados_add.PageID; // for backward compatibility

// extend page with ValidateForm function
z_estados_add.ValidateForm = function(fobj) {
...
z_estados_add.Form_CustomValidate =  
...
z_estados_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
z_estados_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $z_estados->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $z_estados->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $z_estados_add->ShowPageHeader(); ?>
<?php
$z_estados_add->ShowMessage();
?>
<form name="fz_estadosadd" id="fz_estadosadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return z_estados_add.ValidateForm(this);">
<p>
...
<input type="text" name="x_id2_estado" id="x_id2_estado" size="30" value="<?php echo $z_estados->id2_estado->EditValue ?>"<?php echo $z_estados->id2_estado->EditAttributes() ?>>
...
class cz_estados_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			$z_estados->CurrentAction = $_POST["a_add"]; // Get form action
			$this->CopyRecord = $this->LoadOldRecord(); // Load old recordset
			$this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$z_estados->CurrentAction = "I"; // Form error, reset action
			...
			if (@$_GET["id2_estado"] != "") {
				...
				$z_estados->setKey("id2_estado", $z_estados->id2_estado->CurrentValue); // Set up key
			} else {
				$z_estados->setKey("id2_estado", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
				$z_estados->CurrentAction = "C"; // Copy record
			} else {
				$z_estados->CurrentAction = "I"; // Display blank record
		...
		switch ($z_estados->CurrentAction) {
			...
			case "A": // ' Add new record
				$z_estados->SendEmail = TRUE; // Send email on add success
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					$this->setSuccessMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $z_estados->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "z_estadosview.php")
						$sReturnUrl = $z_estados->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$z_estados->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $z_estados;
		$z_estados->nombre->CurrentValue = NULL;
		$z_estados->nombre->OldValue = $z_estados->nombre->CurrentValue;
		$z_estados->lf_estado_pais->CurrentValue = NULL;
		$z_estados->lf_estado_pais->OldValue = $z_estados->lf_estado_pais->CurrentValue;
		$z_estados->visibilidad->CurrentValue = NULL;
		$z_estados->visibilidad->OldValue = $z_estados->visibilidad->CurrentValue;
		$z_estados->id2_estado->CurrentValue = NULL;
		$z_estados->id2_estado->OldValue = $z_estados->id2_estado->CurrentValue;
	}
	...
	function RenderRow() {
		...
		if ($z_estados->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($z_estados->RowType == EW_ROWTYPE_ADD) { // Add row
			...
			$z_estados->id2_estado->EditValue = ew_HtmlEncode($z_estados->id2_estado->CurrentValue);
	...
	function AddRow($rsold = NULL) {
		...
		if ($z_estados->id2_estado->CurrentValue == "" && $z_estados->id2_estado->getSessionValue() == "") {
		...
		$sFilter = $z_estados->KeyFilter();
		if ($bCheckKey) {
			$rsChk = $z_estados->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		// nombre
		$z_estados->nombre->SetDbValueDef($rsnew, $z_estados->nombre->CurrentValue, "", FALSE);

		// lf_estado_pais
		$z_estados->lf_estado_pais->SetDbValueDef($rsnew, $z_estados->lf_estado_pais->CurrentValue, "", FALSE);

		// visibilidad
		$z_estados->visibilidad->SetDbValueDef($rsnew, $z_estados->visibilidad->CurrentValue, 0, FALSE);

		// id2_estado
		$z_estados->id2_estado->SetDbValueDef($rsnew, $z_estados->id2_estado->CurrentValue, 0, FALSE);

		// Call Row Inserting event
		$rs = ($rsold == NULL) ? NULL : $rsold->fields;
		$bInsertRow = $z_estados->Row_Inserting($rs, $rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($z_estados->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			...
			$rs = ($rsold == NULL) ? NULL : $rsold->fields;
			$z_estados->Row_Inserted($rs, $rsnew);
		}
		return $AddRow;
	}
