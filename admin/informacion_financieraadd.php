...
// Create page object
$informacion_financiera_add = new cinformacion_financiera_add();
$Page =& $informacion_financiera_add;

// Page init
$informacion_financiera_add->Page_Init();

// Page main
$informacion_financiera_add->Page_Main();
?>
...
// Create page object
var informacion_financiera_add = new ew_Page("informacion_financiera_add");

// page properties
informacion_financiera_add.PageID = "add"; // page ID
informacion_financiera_add.FormID = "finformacion_financieraadd"; // form ID
var EW_PAGE_ID = informacion_financiera_add.PageID; // for backward compatibility

// extend page with ValidateForm function
informacion_financiera_add.ValidateForm = function(fobj) {
...
informacion_financiera_add.Form_CustomValidate =  
...
informacion_financiera_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
informacion_financiera_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $informacion_financiera->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $informacion_financiera->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $informacion_financiera_add->ShowPageHeader(); ?>
<?php
$informacion_financiera_add->ShowMessage();
?>
<form name="finformacion_financieraadd" id="finformacion_financieraadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return informacion_financiera_add.ValidateForm(this);">
<p>
...
<?php
$informacion_financiera_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$informacion_financiera_add->Page_Terminate();
?>
<?php
...
// Page class
//
class cinformacion_financiera_add {
	...
	// Page object name
	var $PageObjName = 'informacion_financiera_add';
	...
	//
	function cinformacion_financiera_add() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			$informacion_financiera->CurrentAction = $_POST["a_add"]; // Get form action
			$this->CopyRecord = $this->LoadOldRecord(); // Load old recordset
			$this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$informacion_financiera->CurrentAction = "I"; // Form error, reset action
			...
			if (@$_GET["id_informacion_financiera"] != "") {
				...
				$informacion_financiera->setKey("id_informacion_financiera", $informacion_financiera->id_informacion_financiera->CurrentValue); // Set up key
			} else {
				$informacion_financiera->setKey("id_informacion_financiera", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
				$informacion_financiera->CurrentAction = "C"; // Copy record
			} else {
				$informacion_financiera->CurrentAction = "I"; // Display blank record
		...
		switch ($informacion_financiera->CurrentAction) {
			...
			case "A": // ' Add new record
				$informacion_financiera->SendEmail = TRUE; // Send email on add success
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					$this->setSuccessMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $informacion_financiera->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "informacion_financieraview.php")
						$sReturnUrl = $informacion_financiera->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$informacion_financiera->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $informacion_financiera;
		$informacion_financiera->ano->CurrentValue = NULL;
		$informacion_financiera->ano->OldValue = $informacion_financiera->ano->CurrentValue;
		$informacion_financiera->mes->CurrentValue = NULL;
		$informacion_financiera->mes->OldValue = $informacion_financiera->mes->CurrentValue;
		$informacion_financiera->archivo->Upload->DbValue = NULL;
		$informacion_financiera->archivo->OldValue = $informacion_financiera->archivo->Upload->DbValue;
		$informacion_financiera->archivo->CurrentValue = NULL; // Clear file related field
		$informacion_financiera->estatus->CurrentValue = NULL;
		$informacion_financiera->estatus->OldValue = $informacion_financiera->estatus->CurrentValue;
	}
	...
	function RenderRow() {
		...
		} elseif ($informacion_financiera->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow($rsold = NULL) {
		...
		// ano
		$informacion_financiera->ano->SetDbValueDef($rsnew, $informacion_financiera->ano->CurrentValue, 0, FALSE);

		// mes
		$informacion_financiera->mes->SetDbValueDef($rsnew, $informacion_financiera->mes->CurrentValue, "", FALSE);
		...
		// estatus
		$informacion_financiera->estatus->SetDbValueDef($rsnew, $informacion_financiera->estatus->CurrentValue, 0, FALSE);

		// Call Row Inserting event
		$rs = ($rsold == NULL) ? NULL : $rsold->fields;
		$bInsertRow = $informacion_financiera->Row_Inserting($rs, $rsnew);
		if ($bInsertRow) {
			...
			$AddRow = $conn->Execute($informacion_financiera->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$informacion_financiera->id_informacion_financiera->setDbValue($conn->Insert_ID());
			$rsnew['id_informacion_financiera'] = $informacion_financiera->id_informacion_financiera->DbValue;
		}
		if ($AddRow) {

			// Call Row Inserted event
			$rs = ($rsold == NULL) ? NULL : $rsold->fields;
			$informacion_financiera->Row_Inserted($rs, $rsnew);
