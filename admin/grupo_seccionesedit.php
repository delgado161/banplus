...
// Create page object
$grupo_secciones_edit = new cgrupo_secciones_edit();
$Page =& $grupo_secciones_edit;

// Page init
$grupo_secciones_edit->Page_Init();

// Page main
$grupo_secciones_edit->Page_Main();
?>
...
// Create page object
var grupo_secciones_edit = new ew_Page("grupo_secciones_edit");

// page properties
grupo_secciones_edit.PageID = "edit"; // page ID
grupo_secciones_edit.FormID = "fgrupo_seccionesedit"; // form ID
var EW_PAGE_ID = grupo_secciones_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
grupo_secciones_edit.ValidateForm = function(fobj) {
...
grupo_secciones_edit.Form_CustomValidate =  
...
grupo_secciones_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
grupo_secciones_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $grupo_secciones->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $grupo_secciones->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $grupo_secciones_edit->ShowPageHeader(); ?>
<?php
$grupo_secciones_edit->ShowMessage();
?>
<form name="fgrupo_seccionesedit" id="fgrupo_seccionesedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return grupo_secciones_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="grupo_secciones">
...
<input type="hidden" name="x_id_grupo_seccion" id="x_id_grupo_seccion" value="<?php echo ew_HtmlEncode($grupo_secciones->id_grupo_seccion->CurrentValue) ?>">
<p>
<input type="submit" name="btnAction" id="btnAction" value="<?php echo ew_BtnCaption($Language->Phrase("EditBtn")) ?>">
</form>
<?php
$grupo_secciones_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$grupo_secciones_edit->Page_Terminate();
?>
<?php
...
class cgrupo_secciones_edit {
	...
	// Page object name
	var $PageObjName = 'grupo_secciones_edit';
	...
	//
	function cgrupo_secciones_edit() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$grupo_secciones->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$grupo_secciones->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$grupo_secciones->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$grupo_secciones->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($grupo_secciones->id_grupo_seccion->CurrentValue == "")
			$this->Page_Terminate("grupo_seccioneslist.php"); // Invalid key, return to list
		switch ($grupo_secciones->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$grupo_secciones->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$grupo_secciones->RowType = EW_ROWTYPE_EDIT; // Render as Edit
	...
	function LoadFormValues() {
		...
		if (!$grupo_secciones->id_grupo_seccion->FldIsDetailKey)
