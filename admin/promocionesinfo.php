...
//
// Table class for promociones
//
class cpromociones {
	var $TableVar = 'promociones';
	...
	//
	function cpromociones() {
		global $Language;
		$this->AllowAddDeleteRow = ew_AllowAddDeleteRow(); // Allow add/delete row

		// id_promociones
		$this->id_promociones = new cField('promociones', 'promociones', 'x_id_promociones', 'id_promociones', '`id_promociones`', 3, -1, FALSE, '`id_promociones`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_promociones->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_promociones'] =& $this->id_promociones;

		// titulo
		$this->titulo = new cField('promociones', 'promociones', 'x_titulo', 'titulo', '`titulo`', 200, -1, FALSE, '`titulo`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['titulo'] =& $this->titulo;

		// contenido
		$this->contenido = new cField('promociones', 'promociones', 'x_contenido', 'contenido', '`contenido`', 201, -1, FALSE, '`contenido`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->contenido->TruncateMemoRemoveHtml = TRUE;
		$this->fields['contenido'] =& $this->contenido;

		// imagen
		$this->imagen = new cField('promociones', 'promociones', 'x_imagen', 'imagen', '`imagen`', 200, -1, TRUE, '`imagen`', FALSE, FALSE, 'IMAGE');
		$this->imagen->UploadPath = "../uploads/contenidos/";
		$this->fields['imagen'] =& $this->imagen;

		// estatus
		$this->estatus = new cField('promociones', 'promociones', 'x_estatus', 'estatus', '`estatus`', 3, -1, FALSE, '`estatus`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->estatus->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['estatus'] =& $this->estatus;

		// tipo
		$this->tipo = new cField('promociones', 'promociones', 'x_tipo', 'tipo', '`tipo`', 3, -1, FALSE, '`tipo`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->tipo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['tipo'] =& $this->tipo;

		// url
		$this->url = new cField('promociones', 'promociones', 'x_url', 'url', '`url`', 201, -1, FALSE, '`url`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `promociones` ($names) VALUES ($values)";
	}
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `promociones` WHERE ";
		$SQL .= ew_QuotedName('id_promociones') . '=' . ew_QuotedValue($rs['id_promociones'], $this->id_promociones->FldDataType) . ' AND ';
	...
	function SqlKeyFilter() {
		return "`id_promociones` = @id_promociones@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->id_promociones->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_promociones@", ew_AdjustSql($this->id_promociones->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("promocionesview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "promocionesadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("promocionesedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("promocionesadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("promocionesdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_promociones->CurrentValue)) {
			$sUrl .= "id_promociones=" . urlencode($this->id_promociones->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=promociones" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["id_promociones"]; // id_promociones
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_promociones->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		$this->id_promociones->setDbValue($rs->fields('id_promociones'));
   ...
   // Common render codes
		...
		$this->id_promociones->ViewValue = $this->id_promociones->CurrentValue;
		$this->id_promociones->ViewCustomAttributes = "";
		...
		// id_promociones
		$this->id_promociones->LinkCustomAttributes = "";
		$this->id_promociones->HrefValue = "";
		$this->id_promociones->TooltipValue = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('id_promociones', $this->id_promociones->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('titulo', $this->titulo->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('imagen', $this->imagen->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('estatus', $this->estatus->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('tipo', $this->tipo->ExportValue($this->Export, $this->ExportOriginalValue));
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->id_promociones);
				$Doc->ExportCaption($this->titulo);
				$Doc->ExportCaption($this->imagen);
				$Doc->ExportCaption($this->estatus);
				$Doc->ExportCaption($this->tipo);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->id_promociones);
					$Doc->ExportField($this->titulo);
					$Doc->ExportField($this->imagen);
					$Doc->ExportField($this->estatus);
					$Doc->ExportField($this->tipo);
