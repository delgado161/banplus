...
// Create page object
$servicios_delete = new cservicios_delete();
$Page =& $servicios_delete;
...
// Create page object
var servicios_delete = new ew_Page("servicios_delete");

// page properties
servicios_delete.PageID = "delete"; // page ID
servicios_delete.FormID = "fserviciosdelete"; // form ID
var EW_PAGE_ID = servicios_delete.PageID; // for backward compatibility

// extend page with Form_CustomValidate function
servicios_delete.Form_CustomValidate =  
...
servicios_delete.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
servicios_delete.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if ($servicios_delete->Recordset = $servicios_delete->LoadRecordset())
	$servicios_deleteTotalRecs = $servicios_delete->Recordset->RecordCount(); // Get record count
if ($servicios_deleteTotalRecs <= 0) { // No record found, exit
	if ($servicios_delete->Recordset)
		$servicios_delete->Recordset->Close();
	$servicios_delete->Page_Terminate("servicioslist.php"); // Return to list
}
?>
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Delete") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $servicios->TableCaption() ?></p>
...
<?php echo $servicios->TableCustomInnerHtml ?>
	...
	<tr class="ewTableHeader">
		<td valign="top"><?php echo $servicios->nombre->FldCaption() ?></td>
		<td valign="top"><?php echo $servicios->imagen->FldCaption() ?></td>
		<td valign="top"><?php echo $servicios->estatus->FldCaption() ?></td>
	</tr>
	</thead>
	<tbody>
<?php
...
class cservicios_delete {
	...
	function Page_Main() {
		...
		// Load key parameters
		$this->RecKeys = $servicios->GetRecordKeys(); // Load record keys
		$sFilter = $servicios->GetKeyFilter();
		if ($sFilter == "")
		...
		// Get action
		if (@$_POST["a_delete"] <> "") {
			$servicios->CurrentAction = $_POST["a_delete"];
		} else {
			$servicios->CurrentAction = "I"; // Display record
		}
		switch ($servicios->CurrentAction) {
			case "D": // Delete
				$servicios->SendEmail = TRUE; // Send email on delete success
	...
	function WriteAuditTrailOnDelete(&$rs) {
	  ...
	  $curUser = CurrentUserName();
		foreach (array_keys($rs) as $fldname) {
			if (array_key_exists($fldname, $servicios->fields) && $servicios->fields[$fldname]->FldDataType <> EW_DATATYPE_BLOB) { // Ignore BLOB fields
