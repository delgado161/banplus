...
//
// Table class for _ciudad
//
class cz_ciudad {
	var $TableVar = 'z_ciudad';
	...
	function cz_ciudad() {
		...
		// id_ciudad
		$this->id_ciudad = new cField('z_ciudad', '_ciudad', 'x_id_ciudad', 'id_ciudad', '`id_ciudad`', 3, -1, FALSE, '`id_ciudad`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_ciudad->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_ciudad'] =& $this->id_ciudad;

		// lf_estado
		$this->lf_estado = new cField('z_ciudad', '_ciudad', 'x_lf_estado', 'lf_estado', '`lf_estado`', 3, -1, FALSE, '`lf_estado`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->lf_estado->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['lf_estado'] =& $this->lf_estado;

		// id_banplus
		$this->id_banplus = new cField('z_ciudad', '_ciudad', 'x_id_banplus', 'id_banplus', '`id_banplus`', 3, -1, FALSE, '`id_banplus`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_banplus->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_banplus'] =& $this->id_banplus;

		// ciudad
		$this->ciudad = new cField('z_ciudad', '_ciudad', 'x_ciudad', 'ciudad', '`ciudad`', 200, -1, FALSE, '`ciudad`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ciudad'] =& $this->ciudad;

		// visibilidad
		$this->visibilidad = new cField('z_ciudad', '_ciudad', 'x_visibilidad', 'visibilidad', '`visibilidad`', 3, -1, FALSE, '`visibilidad`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `_ciudad` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `_ciudad` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `_ciudad` WHERE ";
		$SQL .= ew_QuotedName('id_ciudad') . '=' . ew_QuotedValue($rs['id_ciudad'], $this->id_ciudad->FldDataType) . ' AND ';
	...
	function SqlKeyFilter() {
		return "`id_ciudad` = @id_ciudad@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->id_ciudad->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_ciudad@", ew_AdjustSql($this->id_ciudad->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("z_ciudadview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "z_ciudadadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("z_ciudadedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("z_ciudadadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("z_ciudaddelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_ciudad->CurrentValue)) {
			$sUrl .= "id_ciudad=" . urlencode($this->id_ciudad->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=z_ciudad" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["id_ciudad"]; // id_ciudad
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_ciudad->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->id_banplus->setDbValue($rs->fields('id_banplus'));
   ...
   // Common render codes
		...
		// id_banplus
		$this->id_banplus->ViewValue = $this->id_banplus->CurrentValue;
		$this->id_banplus->ViewCustomAttributes = "";
		...
		// id_banplus
		$this->id_banplus->LinkCustomAttributes = "";
		$this->id_banplus->HrefValue = "";
		$this->id_banplus->TooltipValue = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('id_banplus', $this->id_banplus->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('ciudad', $this->ciudad->ExportValue($this->Export, $this->ExportOriginalValue));
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->id_banplus);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->id_banplus);
