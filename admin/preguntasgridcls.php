...
class cpreguntas_grid {
	...
	// Page object name
	var $PageObjName = 'preguntas_grid';
...
//		$GLOBALS["Table"] =& $GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			if ($preguntas->AllowAddDeleteRow) {
	...
	function ClearInlineMode() {
		global $preguntas;
		$preguntas->LastAction = $preguntas->CurrentAction; // Save last action
		$preguntas->CurrentAction = ""; // Clear action
	...
	function GridUpdate() {
		global $conn, $Language, $objForm, $gsFormError, $preguntas;
		$bGridUpdate = TRUE;
		$this->WriteAuditTrailDummy($Language->Phrase("BatchUpdateBegin")); // Batch update begin

		// Get old recordset
		$preguntas->CurrentFilter = $this->BuildKeyFilter();
		$sSql = $preguntas->SQL();
				...
				// Validate form and insert/update/delete record
					...
					if ($rowaction == "delete") {
						...
						if ($rowaction == "insert") {
							...
							if ($rowkey <> "") {
								$preguntas->SendEmail = FALSE; // Do not send email on update success
		...
		if ($bGridUpdate) {
			...
			$preguntas->EventCancelled = TRUE; // Set event cancelled
			$preguntas->CurrentAction = "gridedit"; // Stay in Grid Edit mode
		}
		return $bGridUpdate;
	}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$preguntas->id_pregunta->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($preguntas->id_pregunta->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function GridInsert() {
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				$addcnt++;
				$preguntas->SendEmail = FALSE; // Do not send email on insert success
				...
				if ($bGridInsert) {
					...
					$sKey .= $preguntas->id_pregunta->CurrentValue;
		...
		if ($bGridInsert) {
			...
			// Get new recordset
			$preguntas->CurrentFilter = $sWrkFilter;
			$sSql = $preguntas->SQL();
			if ($rs = $conn->Execute($sSql)) {
				$rsnew = $rs->GetRows();
				$rs->Close();
			...
			$preguntas->EventCancelled = TRUE; // Set event cancelled
			$preguntas->CurrentAction = "gridadd"; // Stay in gridadd mode
		}
		return $bGridInsert;
	}
	...
	// Check if empty row
	function EmptyRow() {
		global $preguntas, $objForm;
		if ($objForm->HasValue("x_id_encuesta") && $objForm->HasValue("o_id_encuesta") && $preguntas->id_encuesta->CurrentValue <> $preguntas->id_encuesta->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_pregunta") && $objForm->HasValue("o_pregunta") && $preguntas->pregunta->CurrentValue <> $preguntas->pregunta->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_estatus") && $objForm->HasValue("o_estatus") && $preguntas->estatus->CurrentValue <> $preguntas->estatus->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if (is_numeric($this->RowIndex)) {
			...
			if ($this->RowAction == "insert" && $preguntas->CurrentAction == "F" && $this->EmptyRow())
		...
		if ($preguntas->AllowAddDeleteRow) {
			if ($preguntas->CurrentMode == "add" || $preguntas->CurrentMode == "copy" || $preguntas->CurrentMode == "edit") {
				...
				if (!$Security->CanDelete() && is_numeric($this->RowIndex) && ($this->RowAction == "" || $this->RowAction == "edit")) { // Do not allow delete existing record
					...
					$oListOpt->Body = "<a class=\"ewGridLink\" href=\"javascript:void(0);\" onclick=\"ew_DeleteGridRow(this, preguntas_grid, " . $this->RowIndex . ");\">" . $Language->Phrase("DeleteLink") . "</a>";
				}
			}
		}
		if ($preguntas->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . $preguntas->id_pregunta->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		global $preguntas;
		$preguntas->id_encuesta->CurrentValue = NULL;
		$preguntas->id_encuesta->OldValue = $preguntas->id_encuesta->CurrentValue;
		$preguntas->pregunta->CurrentValue = NULL;
		$preguntas->pregunta->OldValue = $preguntas->pregunta->CurrentValue;
		$preguntas->estatus->CurrentValue = NULL;
		$preguntas->estatus->OldValue = $preguntas->estatus->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$preguntas->id_encuesta->setOldValue($objForm->GetValue("o_id_encuesta"));
		if (!$preguntas->pregunta->FldIsDetailKey) {
			$preguntas->pregunta->setFormValue($objForm->GetValue("x_pregunta"));
		}
		$preguntas->pregunta->setOldValue($objForm->GetValue("o_pregunta"));
		if (!$preguntas->estatus->FldIsDetailKey) {
			$preguntas->estatus->setFormValue($objForm->GetValue("x_estatus"));
		}
		$preguntas->estatus->setOldValue($objForm->GetValue("o_estatus"));
		if (!$preguntas->id_pregunta->FldIsDetailKey && $preguntas->CurrentAction <> "gridadd" && $preguntas->CurrentAction <> "add")
	...
	function RestoreFormValues() {
		global $objForm, $preguntas;
		if ($preguntas->CurrentAction <> "gridadd" && $preguntas->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$preguntas->id_pregunta->CurrentValue = strval($arKeys[0]); // id_pregunta
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		} elseif ($preguntas->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	//
	function DeleteRows() {
		global $conn, $Language, $Security, $preguntas;
		$DeleteRows = TRUE;
		$sSql = $preguntas->SQL();
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$DeleteRows = $preguntas->Row_Deleting($row);
				if (!$DeleteRows) break;
			}
		}
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['id_pregunta'];
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$DeleteRows = $conn->Execute($preguntas->DeleteSQL($row)); // Delete
				$conn->raiseErrorFn = '';
				if ($DeleteRows === FALSE)
					break;
				if ($sKey <> "") $sKey .= ", ";
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($preguntas->getCurrentMasterTable() == "encuesta") {
				$preguntas->id_encuesta->CurrentValue = $preguntas->id_encuesta->getSessionValue();
			}
		$rsnew = array();

		// id_encuesta
		$preguntas->id_encuesta->SetDbValueDef($rsnew, $preguntas->id_encuesta->CurrentValue, NULL, FALSE);

		// pregunta
		$preguntas->pregunta->SetDbValueDef($rsnew, $preguntas->pregunta->CurrentValue, NULL, FALSE);

		// estatus
		$preguntas->estatus->SetDbValueDef($rsnew, $preguntas->estatus->CurrentValue, NULL, FALSE);

		// Call Row Inserting event
		$rs = ($rsold == NULL) ? NULL : $rsold->fields;
		$bInsertRow = $preguntas->Row_Inserting($rs, $rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($preguntas->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
			if ($preguntas->CancelMessage <> "") {
				$this->setFailureMessage($preguntas->CancelMessage);
		...
		if ($AddRow) {
			$preguntas->id_pregunta->setDbValue($conn->Insert_ID());
			$rsnew['id_pregunta'] = $preguntas->id_pregunta->DbValue;
		}
		if ($AddRow) {

			// Call Row Inserted event
			$rs = ($rsold == NULL) ? NULL : $rsold->fields;
			$preguntas->Row_Inserted($rs, $rsnew);
	...
	function SetUpMasterParms() {
		...
		// Hide foreign keys
		$sMasterTblVar = $preguntas->getCurrentMasterTable();
		if ($sMasterTblVar == "encuesta") {
			$preguntas->id_encuesta->Visible = FALSE;
			if ($GLOBALS["encuesta"]->EventCancelled) $preguntas->EventCancelled = TRUE;
		}
...
//		exit();
	  ...
	  $usr = CurrentUserName();
		foreach (array_keys($rs) as $fldname) {
			if ($preguntas->fields[$fldname]->FldDataType <> EW_DATATYPE_BLOB) { // Ignore BLOB fields
				if ($preguntas->fields[$fldname]->FldDataType == EW_DATATYPE_MEMO) {
					if (EW_AUDIT_TRAIL_TO_DATABASE)
						$newvalue = $rs[$fldname];
					else
						$newvalue = "[MEMO]"; // Memo Field
				} elseif ($preguntas->fields[$fldname]->FldDataType == EW_DATATYPE_XML) {
	...
	function WriteAuditTrailOnDelete(&$rs) {
	  ...
	  $curUser = CurrentUserName();
		foreach (array_keys($rs) as $fldname) {
			if (array_key_exists($fldname, $preguntas->fields) && $preguntas->fields[$fldname]->FldDataType <> EW_DATATYPE_BLOB) { // Ignore BLOB fields
				if ($preguntas->fields[$fldname]->FldDataType == EW_DATATYPE_MEMO) {
					if (EW_AUDIT_TRAIL_TO_DATABASE)
						$oldvalue = $rs[$fldname];
					else
