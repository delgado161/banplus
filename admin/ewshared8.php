...
class cMenu {
	...
	function Render($ret = FALSE) {
		...
		$gcnt = 0; // Group count
		$i = 0; // Menu item count
		//$classfot = " class=\"first-of-type\"".$this->Id; //clase de los UL
			...
			if ($this->RenderItem($item)) {
				...
				if ($item->GroupTitle && EW_MENU_ITEM_CLASSNAME <> "") { // Group title
					...
					if (!is_null($item->SubMenu)) {
						foreach ($item->SubMenu->ItemData as $subitem) {
							if ($this->RenderItem($subitem))
								$str .= $subitem->Render($aclass, $liclass,$menu) . "\n"; // Create <LI>
					...
					if ($this->IsRoot && $i == 1){ // For horizontal menu
						$liclass .= " first-of-type".$i;
						}
						$menu=0;
						
			if ($this->Id <> "") {
			  if (!is_numeric($this->Id)){ 
				 $liclass = "Otra_CLase".$i;
				 $aclass = "Iconos".$i;
				 $menu=1;
				  }
				else 
				$liclass="aqui".$i;
					...
					$str .= $item->Render($aclass, $liclass,$aclass,$menu) . "\n"; // Create <LI>
					//echo("<br>render:".$this->Id." menu:".$menu);
...
class cMenuItem {
	...
	// Render
	function Render($aclass = "", $liclass = "",$menu) {

		// Create <A>
		
		if ($this->Url ==  "agenciaslist.php")
			$this->Url = $this->Url."?order=nombre&ordertype=ASC";
