...
// Create page object
$estado_view = new cestado_view();
$Page =& $estado_view;

// Page init
$estado_view->Page_Init();

// Page main
$estado_view->Page_Main();
?>
...
// Create page object
var estado_view = new ew_Page("estado_view");

// page properties
estado_view.PageID = "view"; // page ID
estado_view.FormID = "festadoview"; // form ID
var EW_PAGE_ID = estado_view.PageID; // for backward compatibility

// extend page with Form_CustomValidate function
estado_view.Form_CustomValidate =  
...
estado_view.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
estado_view.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php } ?>
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("View") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $estado->TableCaption() ?>
&nbsp;&nbsp;<?php $estado_view->ExportOptions->Render("body"); ?>
</p>
<?php if ($estado->Export == "") { ?>
<p class="phpmaker">
<a href="<?php echo $estado_view->ListUrl ?>"><?php echo $Language->Phrase("BackToList") ?></a>&nbsp;
<?php if ($Security->CanAdd()) { ?>
<a href="<?php echo $estado_view->AddUrl ?>"><?php echo $Language->Phrase("ViewPageAddLink") ?></a>&nbsp;
<?php } ?>
<?php if ($Security->CanEdit()) { ?>
<a href="<?php echo $estado_view->EditUrl ?>"><?php echo $Language->Phrase("ViewPageEditLink") ?></a>&nbsp;
<?php } ?>
<?php if ($Security->CanDelete()) { ?>
<a href="<?php echo $estado_view->DeleteUrl ?>"><?php echo $Language->Phrase("ViewPageDeleteLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
</p>
...
<?php if ($estado->nombre->Visible) { // nombre ?>
	<tr id="r_nombre"<?php echo $estado->RowAttributes() ?>>
		<td class="ewTableHeader"><?php echo $estado->nombre->FldCaption() ?></td>
		<td<?php echo $estado->nombre->CellAttributes() ?>>
<div<?php echo $estado->nombre->ViewAttributes() ?>><?php echo $estado->nombre->ViewValue ?></div></td>
	</tr>
<?php } ?>
<?php if ($estado->estatus->Visible) { // estatus ?>
	<tr id="r_estatus"<?php echo $estado->RowAttributes() ?>>
		<td class="ewTableHeader"><?php echo $estado->estatus->FldCaption() ?></td>
		<td<?php echo $estado->estatus->CellAttributes() ?>>
<div<?php echo $estado->estatus->ViewAttributes() ?>><?php echo $estado->estatus->ViewValue ?></div></td>
	</tr>
<?php } ?>
</table>
</div>
...
class cestado_view {
	...
	function cestado_view() {
		...
		$KeyUrl = "";
		if (@$_GET["id_estado"] <> "") {
			$this->RecKey["id_estado"] = $_GET["id_estado"];
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id_estado"] <> "") {
				...
				$this->RecKey["id_estado"] = $estado->id_estado->QueryStringValue;
			} else {
				$sReturnUrl = "estadolist.php"; // Return to list
			}

			// Get action
			$estado->CurrentAction = "I"; // Display form
			switch ($estado->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "estadolist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "estadolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
			$this->Page_Terminate($sReturnUrl);

		// Render row
		$estado->RowType = EW_ROWTYPE_VIEW;
		$estado->ResetAttrs();
		$this->RenderRow();
	}
