...
// Create page object
$z_ciudad_edit = new cz_ciudad_edit();
$Page =& $z_ciudad_edit;

// Page init
$z_ciudad_edit->Page_Init();

// Page main
$z_ciudad_edit->Page_Main();
?>
...
// Create page object
var z_ciudad_edit = new ew_Page("z_ciudad_edit");

// page properties
z_ciudad_edit.PageID = "edit"; // page ID
z_ciudad_edit.FormID = "fz_ciudadedit"; // form ID
var EW_PAGE_ID = z_ciudad_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
z_ciudad_edit.ValidateForm = function(fobj) {
...
z_ciudad_edit.Form_CustomValidate =  
...
z_ciudad_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
z_ciudad_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $z_ciudad->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $z_ciudad->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $z_ciudad_edit->ShowPageHeader(); ?>
<?php
$z_ciudad_edit->ShowMessage();
?>
<form name="fz_ciudadedit" id="fz_ciudadedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return z_ciudad_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="z_ciudad">
...
<?php if ($z_ciudad->id_ciudad->Visible) { // id_ciudad ?>
	<tr id="r_id_ciudad"<?php echo $z_ciudad->RowAttributes() ?>>
		...
		<td<?php echo $z_ciudad->id_ciudad->CellAttributes() ?>><span id="el_id_ciudad">
<div<?php echo $z_ciudad->id_ciudad->ViewAttributes() ?>><?php echo $z_ciudad->id_ciudad->EditValue ?></div>
<input type="hidden" name="x_id_ciudad" id="x_id_ciudad" value="<?php echo ew_HtmlEncode($z_ciudad->id_ciudad->CurrentValue) ?>">
</span><?php echo $z_ciudad->id_ciudad->CustomMsg ?></td>
	</tr>
<?php } ?>
...
class cz_ciudad_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$z_ciudad->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$z_ciudad->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$z_ciudad->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$z_ciudad->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($z_ciudad->id_ciudad->CurrentValue == "")
			$this->Page_Terminate("z_ciudadlist.php"); // Invalid key, return to list
		switch ($z_ciudad->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$z_ciudad->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$z_ciudad->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$z_ciudad->ResetAttrs();
		$this->RenderRow();
	}
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm, $z_ciudad;
		if (!$z_ciudad->id_ciudad->FldIsDetailKey)
			$z_ciudad->id_ciudad->setFormValue($objForm->GetValue("x_id_ciudad"));
	...
	function RestoreFormValues() {
		global $objForm, $z_ciudad;
		$this->LoadRow();
		$z_ciudad->id_ciudad->CurrentValue = $z_ciudad->id_ciudad->FormValue;
	...
	function RenderRow() {
		...
		} elseif ($z_ciudad->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id_ciudad
			$z_ciudad->id_ciudad->EditCustomAttributes = "";
			$z_ciudad->id_ciudad->EditValue = $z_ciudad->id_ciudad->CurrentValue;
			$z_ciudad->id_ciudad->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// lf_estado
			$z_ciudad->lf_estado->SetDbValueDef($rsnew, $z_ciudad->lf_estado->CurrentValue, 0, $z_ciudad->lf_estado->ReadOnly);

			// id_banplus
			$z_ciudad->id_banplus->SetDbValueDef($rsnew, $z_ciudad->id_banplus->CurrentValue, 0, $z_ciudad->id_banplus->ReadOnly);

			// ciudad
			$z_ciudad->ciudad->SetDbValueDef($rsnew, $z_ciudad->ciudad->CurrentValue, "", $z_ciudad->ciudad->ReadOnly);

			// visibilidad
			$z_ciudad->visibilidad->SetDbValueDef($rsnew, $z_ciudad->visibilidad->CurrentValue, NULL, $z_ciudad->visibilidad->ReadOnly);

			// Call Row Updating event
			$bUpdateRow = $z_ciudad->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				if (count($rsnew) > 0)
					$EditRow = $conn->Execute($z_ciudad->UpdateSQL($rsnew));
				else
		...
		if ($EditRow)
			$z_ciudad->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
