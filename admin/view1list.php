...
<?php
$gsMasterReturnUrl = "banner_seccionlist.php";
if ($view1_list->DbMasterFilter <> "" && $view1->getCurrentMasterTable() == "banner_seccion") {
	if ($view1_list->MasterRecordExists) {
		if ($view1->getCurrentMasterTable() == $view1->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("MasterRecord") ?><?php echo $banner_seccion->TableCaption() ?>
...
<?php } ?>
<?php include_once "banner_seccionmaster.php" ?>
<?php
	}
}
?>
...
<?php if ($view1->getCurrentMasterTable() == "") { ?>
&nbsp;&nbsp;<?php $view1_list->ExportOptions->Render("body"); ?>
<?php } ?>
</p>
...
<?php if ($view1->id_grupo_seccion->Visible) { // id_grupo_seccion ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onmousedown="ew_Sort(event,'<?php echo $view1->SortUrl($view1->id_grupo_seccion) ?>',1);">
...
class cview1_list {
	...
	function BasicSearchSQL($Keyword) {
		...
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $view1->titulo, $Keyword);
		return $sWhere;
	}
	...
	function RestoreSearchParms() {
		global $view1;
		$bRestore = TRUE;
		if ($view1->BasicSearchKeyword <> "") $bRestore = FALSE;
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$view1->UpdateSort($view1->id_grupo_seccion); // id_grupo_seccion
			$view1->UpdateSort($view1->titulo); // titulo
	...
	function ResetCmd() {
		...
		if (@$_GET["cmd"] <> "") {
			...
			if (strtolower($sCmd) == "resetsort") {
				$sOrderBy = "";
				$view1->setSessionOrderBy($sOrderBy);
				$view1->id_grupo_seccion->setSort("");
				$view1->titulo->setSort("");
			}
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($view1->getKey("id_grupo_seccion")) <> "")
			$view1->id_grupo_seccion->CurrentValue = $view1->getKey("id_grupo_seccion"); // id_grupo_seccion
		else
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if (@$_GET[EW_TABLE_SHOW_MASTER] <> "") {
			...
			if ($sMasterTblVar == "banner_seccion") {
				$bValidMaster = TRUE;
				if (@$_GET["id_grupo_secciones"] <> "") {
					$GLOBALS["banner_seccion"]->id_grupo_secciones->setQueryStringValue($_GET["id_grupo_secciones"]);
					$view1->id_grupo_seccion->setQueryStringValue($GLOBALS["banner_seccion"]->id_grupo_secciones->QueryStringValue);
					$view1->id_grupo_seccion->setSessionValue($view1->id_grupo_seccion->QueryStringValue);
					if (!is_numeric($GLOBALS["banner_seccion"]->id_grupo_secciones->QueryStringValue)) $bValidMaster = FALSE;
				} else {
					$bValidMaster = FALSE;
				}
			}
		...
		if ($bValidMaster) {
			...
			$view1->setCurrentMasterTable($sMasterTblVar);
			...
			if ($sMasterTblVar <> "banner_seccion") {
				if ($view1->id_grupo_seccion->QueryStringValue == "") $view1->id_grupo_seccion->setSessionValue("");
			}
		}
