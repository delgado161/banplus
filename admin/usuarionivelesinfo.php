...
//
// Table class for usuarioniveles
//
class cusuarioniveles {
	var $TableVar = 'usuarioniveles';
	...
	//
	function cusuarioniveles() {
		global $Language;
		$this->AllowAddDeleteRow = ew_AllowAddDeleteRow(); // Allow add/delete row

		// userlevelid
		$this->userlevelid = new cField('usuarioniveles', 'usuarioniveles', 'x_userlevelid', 'userlevelid', '`userlevelid`', 3, -1, FALSE, '`userlevelid`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->userlevelid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['userlevelid'] =& $this->userlevelid;

		// userlevelname
		$this->userlevelname = new cField('usuarioniveles', 'usuarioniveles', 'x_userlevelname', 'userlevelname', '`userlevelname`', 200, -1, FALSE, '`userlevelname`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function HighlightName() {
		return "usuarioniveles_Highlight";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `usuarioniveles` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `usuarioniveles` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `usuarioniveles` WHERE ";
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("usuarionivelesview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "usuarionivelesadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("usuarionivelesedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("usuarionivelesadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("usuarionivelesdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=usuarioniveles" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					$XmlDoc->AddField('userlevelname', $this->userlevelname->ExportValue($this->Export, $this->ExportOriginalValue));
				} else {
					$XmlDoc->AddField('userlevelname', $this->userlevelname->ExportValue($this->Export, $this->ExportOriginalValue));
				}
			}
