...
// Create page object
$noticias_edit = new cnoticias_edit();
$Page =& $noticias_edit;
...
noticias_edit.FormID = "fnoticiasedit"; // form ID
var EW_PAGE_ID = noticias_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
noticias_edit.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo "T\00edtulo"  ?>");
...
noticias_edit.Form_CustomValidate =  
...
noticias_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
noticias_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $noticias->TableCaption() ?></p>
...
?>
<form name="fnoticiasedit" id="fnoticiasedit" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return noticias_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="noticias">
...
<?php if ($noticias->noticias_link->Visible) { // noticias_link ?>
	<tr id="r_noticias_link"<?php echo $noticias->RowAttributes() ?>>
		<td class="ewTableHeader"><?php echo $noticias->noticias_link->FldCaption() ?> <?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
...
<input type="hidden" name="x_id_noticia" id="x_id_noticia" value="<?php echo ew_HtmlEncode($noticias->id_noticia->CurrentValue) ?>">
<p>
...
class cnoticias_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$noticias->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$noticias->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$noticias->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$noticias->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($noticias->id_noticia->CurrentValue == "")
		...
		switch ($noticias->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$noticias->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$noticias->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$noticias->ResetAttrs();
		$this->RenderRow();
	}
	...
	function LoadFormValues() {
		...
		if (!$noticias->id_noticia->FldIsDetailKey)
			$noticias->id_noticia->setFormValue($objForm->GetValue("x_id_noticia"));
	...
	function RestoreFormValues() {
		global $objForm, $noticias;
		$this->LoadRow();
		$noticias->id_noticia->CurrentValue = $noticias->id_noticia->FormValue;
	...
	function RenderRow() {
		...
		if ($noticias->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($noticias->RowType == EW_ROWTYPE_EDIT) { // Edit row
			...
			$arwrk[] = array("5", $noticias->tipo->FldTagCaption(5) <> "" ? $noticias->tipo->FldTagCaption(5) : "1");
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// tipo
			$noticias->tipo->SetDbValueDef($rsnew, $noticias->tipo->CurrentValue, 0, $noticias->tipo->ReadOnly);

			// titulo
			$noticias->titulo->SetDbValueDef($rsnew, $noticias->titulo->CurrentValue, NULL, $noticias->titulo->ReadOnly);

			// antetitulo
			$noticias->antetitulo->SetDbValueDef($rsnew, $noticias->antetitulo->CurrentValue, NULL, $noticias->antetitulo->ReadOnly);

			// sumario
			$noticias->sumario->SetDbValueDef($rsnew, $noticias->sumario->CurrentValue, NULL, $noticias->sumario->ReadOnly);

			// contenido
			$noticias->contenido->SetDbValueDef($rsnew, $noticias->contenido->CurrentValue, NULL, $noticias->contenido->ReadOnly);

			// id_grupo
			$noticias->id_grupo->SetDbValueDef($rsnew, $noticias->id_grupo->CurrentValue, "", $noticias->id_grupo->ReadOnly);
			...
			$noticias->imagen->Upload->DbValue = $rs->fields('imagen'); // Get original value
			...
			// fecha
			$noticias->fecha->SetDbValueDef($rsnew, ew_UnFormatDateTime($noticias->fecha->CurrentValue, 7), NULL, $noticias->fecha->ReadOnly);

			// orden
			$noticias->orden->SetDbValueDef($rsnew, $noticias->orden->CurrentValue, NULL, $noticias->orden->ReadOnly);

			// f_desde
			$noticias->f_desde->SetDbValueDef($rsnew, ew_UnFormatDateTime($noticias->f_desde->CurrentValue, 7), ew_CurrentDate(), $noticias->f_desde->ReadOnly);

			// f_hasta
			$noticias->f_hasta->SetDbValueDef($rsnew, ew_UnFormatDateTime($noticias->f_hasta->CurrentValue, 7), ew_CurrentDate(), $noticias->f_hasta->ReadOnly);
			...
			$noticias->archivo->Upload->DbValue = $rs->fields('archivo'); // Get original value
			...
			// noticias_link
			$noticias->noticias_link->SetDbValueDef($rsnew, $noticias->noticias_link->CurrentValue, NULL, $noticias->noticias_link->ReadOnly);

			
			// estatus
			$noticias->estatus->SetDbValueDef($rsnew, $noticias->estatus->CurrentValue, NULL, $noticias->estatus->ReadOnly);

			// Call Row Updating event
			$bUpdateRow = $noticias->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
			...
			if (!ew_Empty($noticias->archivo->Upload->Value)) {
				...
				$conn->raiseErrorFn = 'ew_ErrorFn';
				if (count($rsnew) > 0)
					$EditRow = $conn->Execute($noticias->UpdateSQL($rsnew));
				else
		...
		if ($EditRow)
			$noticias->Row_Updated($rsold, $rsnew);
		if ($EditRow) {
	  ...
	  $usr = CurrentUserName();
		foreach (array_keys($rsnew) as $fldname) {
			if ($noticias->fields[$fldname]->FldDataType <> EW_DATATYPE_BLOB) { // Ignore BLOB fields
				if ($noticias->fields[$fldname]->FldDataType == EW_DATATYPE_DATE) { // DateTime field
					$modified = (ew_FormatDateTime($rsold[$fldname], 0) <> ew_FormatDateTime($rsnew[$fldname], 0));
				} else {
					$modified = !ew_CompareValue($rsold[$fldname], $rsnew[$fldname]);
				}
				if ($modified) {
					if ($noticias->fields[$fldname]->FldDataType == EW_DATATYPE_MEMO) { // Memo field
					...
					} elseif ($noticias->fields[$fldname]->FldDataType == EW_DATATYPE_XML) { // XML field
						$oldvalue = "[XML]";
						$newvalue = "[XML]";
					} else {
						$oldvalue = $rsold[$fldname];
