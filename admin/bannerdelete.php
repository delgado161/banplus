...
banner_delete.FormID = "fbannerdelete"; // form ID
...
if ($banner_delete->Recordset = $banner_delete->LoadRecordset())
	$banner_deleteTotalRecs = $banner_delete->Recordset->RecordCount(); // Get record count
if ($banner_deleteTotalRecs <= 0) { // No record found, exit
	if ($banner_delete->Recordset)
		$banner_delete->Recordset->Close();
	$banner_delete->Page_Terminate("bannerlist.php"); // Return to list
}
?>
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Delete") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $banner->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $banner->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $banner_delete->ShowPageHeader(); ?>
<?php
...
<?php foreach ($banner_delete->RecKeys as $key) { ?>
...
<?php echo $banner->TableCustomInnerHtml ?>
	...
	<tr class="ewTableHeader">
		<td valign="top"><?php echo $banner->nombre->FldCaption() ?></td>
		<td valign="top"><?php echo $banner->imagen->FldCaption() ?></td>
		<td valign="top"><?php echo $banner->estatus->FldCaption() ?></td>
	</tr>
	</thead>
	<tbody>
<?php
...
$i = 0;
while (!$banner_delete->Recordset->EOF) {
	$banner_delete->RecCnt++;
	...
	$banner_delete->LoadRowValues($banner_delete->Recordset);
...
class cbanner_delete {
	...
	function ShowMessage() {
		...
		if ($sErrorMessage <> "") { // Message in Session, display
			...
			if ($pos == true) 
				echo "<p class=\"ewErrorMessage\">No se puede eliminar este banner porque esta relacionado a un contenido</p>";
			else
	...
	function Page_Main() {
		...
		// Load key parameters
		$this->RecKeys = $banner->GetRecordKeys(); // Load record keys
	...
	function WriteAuditTrailOnDelete(&$rs) {
	  ...
	  $curUser = CurrentUserName();
		foreach (array_keys($rs) as $fldname) {
			if (array_key_exists($fldname, $banner->fields) && $banner->fields[$fldname]->FldDataType <> EW_DATATYPE_BLOB) { // Ignore BLOB fields
