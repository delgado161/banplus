...
// Create page object
$preguntas_add = new cpreguntas_add();
$Page =& $preguntas_add;

// Page init
$preguntas_add->Page_Init();

// Page main
$preguntas_add->Page_Main();
?>
...
// Create page object
var preguntas_add = new ew_Page("preguntas_add");

// page properties
preguntas_add.PageID = "add"; // page ID
preguntas_add.FormID = "fpreguntasadd"; // form ID
var EW_PAGE_ID = preguntas_add.PageID; // for backward compatibility

// extend page with ValidateForm function
preguntas_add.ValidateForm = function(fobj) {
...
preguntas_add.Form_CustomValidate =  
...
preguntas_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
preguntas_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $preguntas->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $preguntas->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $preguntas_add->ShowPageHeader(); ?>
<?php
$preguntas_add->ShowMessage();
?>
<form name="fpreguntasadd" id="fpreguntasadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return preguntas_add.ValidateForm(this);">
<p>
...
<p>
<?php if ($preguntas->getCurrentDetailTable() == "respuestas" && $respuestas->DetailAdd) { ?>
<br>
...
class cpreguntas_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			$preguntas->CurrentAction = $_POST["a_add"]; // Get form action
			...
			if (!$this->ValidateForm()) {
				$preguntas->CurrentAction = "I"; // Form error, reset action
			...
			if (@$_GET["id_pregunta"] != "") {
				...
				$preguntas->setKey("id_pregunta", $preguntas->id_pregunta->CurrentValue); // Set up key
			} else {
				$preguntas->setKey("id_pregunta", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
				$preguntas->CurrentAction = "C"; // Copy record
			} else {
				$preguntas->CurrentAction = "I"; // Display blank record
		...
		switch ($preguntas->CurrentAction) {
			...
			case "A": // ' Add new record
				$preguntas->SendEmail = TRUE; // Send email on add success
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					$this->setSuccessMessage($Language->Phrase("AddSuccess")); // Set up success message
					if ($preguntas->getCurrentDetailTable() <> "") // Master/detail add
						$sReturnUrl = $preguntas->getDetailUrl();
					else
						$sReturnUrl = $preguntas->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "preguntasview.php")
						$sReturnUrl = $preguntas->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$preguntas->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function ValidateForm() {
		...
		if ($preguntas->getCurrentDetailTable() == "respuestas" && $GLOBALS["respuestas"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			if ($preguntas->getCurrentDetailTable() == "respuestas" && $GLOBALS["respuestas"]->DetailAdd) {
				$GLOBALS["respuestas"]->id_pregunta->setSessionValue($preguntas->id_pregunta->CurrentValue); // Set master key
				$respuestas_grid = new crespuestas_grid(); // get detail page object
				$AddRow = $respuestas_grid->GridInsert();
				$respuestas_grid = NULL;
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			if ($sDetailTblVar == "respuestas") {
				...
				if ($GLOBALS["respuestas"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["respuestas"]->CurrentMode = "copy";
					else
						$GLOBALS["respuestas"]->CurrentMode = "add";
					$GLOBALS["respuestas"]->CurrentAction = "gridadd";
