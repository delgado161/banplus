...
faq_add.FormID = "ffaqadd"; // form ID
var EW_PAGE_ID = faq_add.PageID; // for backward compatibility

// extend page with ValidateForm function
faq_add.ValidateForm = function(fobj) {
...
faq_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
faq_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $faq->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $faq->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $faq_add->ShowPageHeader(); ?>
<?php
$faq_add->ShowMessage();
?>
<form name="ffaqadd" id="ffaqadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return faq_add.ValidateForm(this);">
<p>
...
class cfaq_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			$faq->CurrentAction = $_POST["a_add"]; // Get form action
			$this->CopyRecord = $this->LoadOldRecord(); // Load old recordset
			$this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$faq->CurrentAction = "I"; // Form error, reset action
			...
			if (@$_GET["id_faq"] != "") {
				...
				$faq->setKey("id_faq", $faq->id_faq->CurrentValue); // Set up key
			} else {
				$faq->setKey("id_faq", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
				$faq->CurrentAction = "C"; // Copy record
			} else {
				$faq->CurrentAction = "I"; // Display blank record
		...
		switch ($faq->CurrentAction) {
			...
			case "A": // ' Add new record
				$faq->SendEmail = TRUE; // Send email on add success
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					$this->setSuccessMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $faq->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "faqview.php")
						$sReturnUrl = $faq->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$faq->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $faq;
		$faq->pregunta->CurrentValue = NULL;
		$faq->pregunta->OldValue = $faq->pregunta->CurrentValue;
		$faq->respuesta->CurrentValue = NULL;
		$faq->respuesta->OldValue = $faq->respuesta->CurrentValue;
		$faq->estatus->CurrentValue = NULL;
		$faq->estatus->OldValue = $faq->estatus->CurrentValue;
	}
	...
	function RenderRow() {
		...
		} elseif ($faq->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow($rsold = NULL) {
		...
		// pregunta
		$faq->pregunta->SetDbValueDef($rsnew, $faq->pregunta->CurrentValue, NULL, FALSE);

		// respuesta
		$faq->respuesta->SetDbValueDef($rsnew, $faq->respuesta->CurrentValue, NULL, FALSE);

		// estatus
		$faq->estatus->SetDbValueDef($rsnew, $faq->estatus->CurrentValue, NULL, FALSE);

		// Call Row Inserting event
		$rs = ($rsold == NULL) ? NULL : $rsold->fields;
		$bInsertRow = $faq->Row_Inserting($rs, $rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($faq->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$faq->id_faq->setDbValue($conn->Insert_ID());
			$rsnew['id_faq'] = $faq->id_faq->DbValue;
		}
		if ($AddRow) {

			// Call Row Inserted event
			$rs = ($rsold == NULL) ? NULL : $rsold->fields;
			$faq->Row_Inserted($rs, $rsnew);
		}
		return $AddRow;
	}
