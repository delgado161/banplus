...
// Create page object
$modulos_edit = new cmodulos_edit();
$Page =& $modulos_edit;

// Page init
$modulos_edit->Page_Init();

// Page main
$modulos_edit->Page_Main();
?>
...
// Create page object
var modulos_edit = new ew_Page("modulos_edit");

// page properties
modulos_edit.PageID = "edit"; // page ID
modulos_edit.FormID = "fmodulosedit"; // form ID
var EW_PAGE_ID = modulos_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
modulos_edit.ValidateForm = function(fobj) {
...
modulos_edit.Form_CustomValidate =  
...
modulos_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
modulos_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $modulos->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $modulos->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $modulos_edit->ShowPageHeader(); ?>
<?php
$modulos_edit->ShowMessage();
?>
<form name="fmodulosedit" id="fmodulosedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return modulos_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="modulos">
...
<input type="hidden" name="x_id_modulo_financiero" id="x_id_modulo_financiero" value="<?php echo ew_HtmlEncode($modulos->id_modulo_financiero->CurrentValue) ?>">
<p>
<input type="submit" name="btnAction" id="btnAction" value="<?php echo ew_BtnCaption($Language->Phrase("EditBtn")) ?>">
</form>
<?php
$modulos_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
class cmodulos_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$modulos->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$modulos->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$modulos->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$modulos->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($modulos->id_modulo_financiero->CurrentValue == "")
			$this->Page_Terminate("moduloslist.php"); // Invalid key, return to list
		switch ($modulos->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$modulos->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$modulos->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$modulos->ResetAttrs();
		$this->RenderRow();
	}
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm, $modulos;
		if (!$modulos->id_modulo_financiero->FldIsDetailKey)
			$modulos->id_modulo_financiero->setFormValue($objForm->GetValue("x_id_modulo_financiero"));
	}
	...
	function RestoreFormValues() {
		global $objForm, $modulos;
		$this->LoadRow();
		$modulos->id_modulo_financiero->CurrentValue = $modulos->id_modulo_financiero->FormValue;
	}
	...
	function RenderRow() {
		...
		} elseif ($modulos->RowType == EW_ROWTYPE_EDIT) { // Edit row
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $modulos->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				if (count($rsnew) > 0)
					$EditRow = $conn->Execute($modulos->UpdateSQL($rsnew));
				else
		...
		if ($EditRow)
			$modulos->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
