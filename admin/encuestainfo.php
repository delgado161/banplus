...
class cencuesta {
	...
	function cencuesta() {
		...
		// id_encuesta
		$this->id_encuesta = new cField('encuesta', 'encuesta', 'x_id_encuesta', 'id_encuesta', '`id_encuesta`', 3, -1, FALSE, '`id_encuesta`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_encuesta->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_encuesta'] =& $this->id_encuesta;

		// nombre
		$this->nombre = new cField('encuesta', 'encuesta', 'x_nombre', 'nombre', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nombre'] =& $this->nombre;

		// id_grupo
		$this->id_grupo = new cField('encuesta', 'encuesta', 'x_id_grupo', 'id_grupo', '`id_grupo`', 200, -1, FALSE, '`id_grupo`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['id_grupo'] =& $this->id_grupo;

		// estatus
		$this->estatus = new cField('encuesta', 'encuesta', 'x_estatus', 'estatus', '`estatus`', 3, -1, FALSE, '`estatus`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "preguntas") {
			$sDetailUrl = $GLOBALS["preguntas"]->ListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&id_encuesta=" . $this->id_encuesta->CurrentValue;
		}
		return $sDetailUrl;
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `encuesta` ($names) VALUES ($values)";
	}
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `encuesta` WHERE ";
		$SQL .= ew_QuotedName('id_encuesta') . '=' . ew_QuotedValue($rs['id_encuesta'], $this->id_encuesta->FldDataType) . ' AND ';
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->id_encuesta->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_encuesta@", ew_AdjustSql($this->id_encuesta->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("encuestaview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "encuestaadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("encuestaedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("encuestaedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function CopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("encuestaadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("encuestaadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("encuestadelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_encuesta->CurrentValue)) {
			$sUrl .= "id_encuesta=" . urlencode($this->id_encuesta->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=encuesta" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["id_encuesta"]; // id_encuesta
	...
	function LoadListRowValues(&$rs) {
		$this->id_encuesta->setDbValue($rs->fields('id_encuesta'));
